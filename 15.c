...
hashmap_t* hm_new(uint32_t size) {
...
uint32_t hm_get(hashmap_t* hm, uint32_t key) {
...
void hm_set(hashmap_t* hm, uint32_t key, uint32_t value) {
  uint32_t index = key % hm->cap;
  u_int8_t tries = 0;
...
static uint32_t parse_input(uint32_t* numbers, char* s) {
  uint32_t num;
  uint32_t n = 0;
...
int day15() {
  ...
  if (!seen) {
    err(EXIT_FAILURE, "could not allocate memory for lookup table");
  }
